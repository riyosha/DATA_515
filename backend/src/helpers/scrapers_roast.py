...
def _extract_movie_details(element) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    if movie_tag and movie_tag.get("href"):
        movie_url = f"https://letterboxd.com{movie_tag['href']}"
    # Extract movie year
    year_tag = element.find("small", class_="metadata")
    year_anchor = year_tag.find("a") if year_tag else None
    movie_year = year_anchor.text.strip() if year_anchor else None
    return movie_name, movie_url, movie_year
...
def _extract_review_details(element) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    return rating, watched_date, review_text
...
def _process_review_page(base_url: str, page: int) -> List[List[Optional[str]]]:
    ...
    """
    page_url = f"{base_url}/films/reviews/page/{page}/"
    page_reviews = []
    try:
        ...
        for element in review_elements:
            movie_name, movie_url, movie_year = _extract_movie_details(element)
            rating, watched_date, review_text = _extract_review_details(element)
            page_reviews.append([
                movie_name,
                movie_url,
                movie_year,
        ...
        # Silently continue if there's an issue with this specific page
        pass
    return page_reviews
...
def letterboxd_user_reviews_scraper(username: str) -> None:
    ...
    """
    base_profile_url = f"https://letterboxd.com/{username}"
    reviews_url = f"{base_profile_url}/films/reviews/"
    ...
    for page in range(1, last_page + 1):
        page_reviews = _process_review_page(base_profile_url, page)
        all_reviews.extend(page_reviews)

    # Write the reviews data to a CSV file
    csv_filename = f"{username}_reviews.csv"
...
def _extract_stats_values(stats_elements) -> Dict[str, str]:
    ...
    if len(stats_elements) < 6:
        raise ScraperError("Unexpected stats format")
    return {
        "num_years": stats_elements[0].text.split()[0],
        "num_hours": stats_elements[1].text.split()[0],
        "num_directors": stats_elements[2].text.split()[0],
        "num_countries": stats_elements[3].text.split()[0],
        "longest_streak": stats_elements[4].text.split()[0],
        "two_plus_film_days": stats_elements[5].text.split()[0]
    }
...
def letterboxd_user_stats_scraper(username: str) -> None:
    ...
    if response.status_code != 200:
        raise ScraperError(
            f"Couldn't find stats for {username}. Status code: {response.status_code}"
        )
    ...
    try:
        stats = _extract_stats_values(stats_elements)
    except ScraperError as error:
        raise ScraperError(f"Unexpected stats format for {username}: {error}") from error

    csv_filename = f"{username}_stats.csv"
    with open(csv_filename, mode="w", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        writer.writerow([
            "Number of Years",
